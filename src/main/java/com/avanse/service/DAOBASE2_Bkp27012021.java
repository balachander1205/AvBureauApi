 ...
 * The DAO class for AvBureauApi api's.
 * <p> This program perform database operation for bureau api's</p>
 * 
 * @author Swapnil Sawant
 * @version 1.0
 * @since 2019-07-22
 */
public class DAOBASE2_Bkp27012021 {

	/**
	 * 
	 * 
	 * @param request User original input request attributes
	 * @return CibilRequest This code converts user inputs into cibil input request to be send to cibil server for processing.
	 */
	public NewCibilRequest getSegmentDataModel(Request request) {
		...
		try {
				...
				hb.setFutureUse1("  ");
				hb.setEnqMemberUserId(fixedLengthString(
				this.getCibilUserID(), 30));
				...
				if (telBeanList.size() < 3 && MOBILE != null
						&& !MOBILE.equals("")) {
					telBeanList.add(new TelephoneBean(MOBILE, null, "01"));
				}

				if (telBeanList.size() < 3 && HOME_PHONE != null
						&& !HOME_PHONE.equals("")) {
					telBeanList.add(new TelephoneBean(HOME_PHONE, null, "02"));
				}

				if (telBeanList.size() < 3 && OFFICE_PHONE != null
						&& !OFFICE_PHONE.equals("")) {
					telBeanList
							.add(new TelephoneBean(OFFICE_PHONE, null, "03"));
				}
				...
				if (fullAddress.trim().length() < 200
						&& !RESI_LANDMARK.equals("")) {
	...
	public String getCibilUserID()
	{
		...
		String query = "select const_value from ilm_gl_const_params where const_code = 'CIBIL_USER_ID'";
		try {
			Connection con = ConnectionFactory.getConnection();
			Statement st = con.createStatement();
			rs = st.executeQuery(query);
	...
	public String getCibilUserPwd()
	{
		...
		String query = "select const_value from ilm_gl_const_params where const_code = 'CIBIL_USER_PWD'";
		try {
			Connection con = ConnectionFactory.getConnection();
			Statement st = con.createStatement();
			rs = st.executeQuery(query);
	...
	public void validateCibilRequest(NewCibilRequest ncr) {
		...
		sb.append(ncr.getAddressSegment()
	...
	public boolean validateFFRRequest(FFRHeader header,FFRRequest request)
	{
		...
		{
			sb.append("SrNo:"+srNo+"/FileNo:"+fileNo+"/CustID:"+custId+":"+"InputCibilString is null or blank");
		}
		if(emailId==null || emailId.equals(""))
		{
			sb.append("SrNo:"+srNo+"/FileNo:"+fileNo+"/CustID:"+custId+":"+"Customer EmailID is null or blank");
		}
		if(name==null || name.equals(""))
		{
			sb.append("SrNo:"+srNo+"/FileNo:"+fileNo+"/CustID:"+custId+":"+"Customer Name is null or blank");
		}
	...
	public Map<FFRHeader,FFRRequest> getFFRDataModel(ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				String fullName=(rs.getString("FNAME")!=null?rs.getString("FNAME"):""+" "+rs.getString("MNAME")!=null?rs.getString("MNAME"):""+" "+rs.getString("LNAME")!=null?rs.getString("LNAME"):"").trim();
	...
	public int updateFFRResponse(FFRHeader header,FFRResponse response,String jsonResponse)
	{
		...
		try {
			System.out.println("query:" + query + " fileNo: " + header.getFileNo()
					+ " custID:" + header.getCustID()+ " srNo:" + header.getSrNo());
			Connection con = ConnectionFactory.getConnection();
	...
	public void storeParsedCibilResponse(CibilResponse cibilResponse,String mobRefNo,String leadID,long srNo)
	{
		...
		if(cibilResponse!=null)
		{
			...
			if(cibilResponse.getIdentificationSegment()!=null && cibilResponse.getIdentificationSegment().getIdBeanList()!=null)
			{
				this.storeIdentificationSegment(cibilResponse.getIdentificationSegment().getIdBeanList(), mobRefNo, leadID,srNo);
			}
			
			if(cibilResponse.getTelephoneSegment()!=null && cibilResponse.getTelephoneSegment().getTelBeanList()!=null)
			{
				this.storeTelephoneSegment(cibilResponse.getTelephoneSegment().getTelBeanList(), mobRefNo, leadID,srNo);
			}
			
			if(cibilResponse.getEmailContactSegment()!=null && cibilResponse.getEmailContactSegment().getEmailList()!=null)
			{
				this.storeEmailContactSegment(cibilResponse.getEmailContactSegment().getEmailList(), mobRefNo, leadID,srNo);
			}
			...
			if(cibilResponse.getAccNumSegment()!=null && cibilResponse.getAccNumSegment().getAccNumBeanList()!=null)
			{
				this.storeEnquiryAccountNumberSegment(cibilResponse.getAccNumSegment().getAccNumBeanList(), mobRefNo, leadID,srNo);
			}
			...
			if(cibilResponse.getAddressSegment()!=null && cibilResponse.getAddressSegment().getAddBeanList()!=null)
			{
				this.storeAddressSegment(cibilResponse.getAddressSegment().getAddBeanList(), mobRefNo, leadID,srNo);	
			}
			
			if(cibilResponse.getAccountSegment()!=null && cibilResponse.getAccountSegment().getOwnAccountBeanList()!=null)
			{
				this.storeAccountSegmentOwnAccounts(cibilResponse.getAccountSegment().getOwnAccountBeanList(), mobRefNo, leadID,srNo);
			}
			
			if(cibilResponse.getAccountSegment()!=null && cibilResponse.getAccountSegment().getOthersAccountBeanList()!=null)
			{
				this.storeAccountSegmentOthersAccounts(cibilResponse.getAccountSegment().getOthersAccountBeanList(), mobRefNo, leadID,srNo);
			}
			
			if(cibilResponse.getEnquirySegment()!=null && cibilResponse.getEnquirySegment().getEnquiryBeanList()!=null)
			{
				this.storeEnquirySegment(cibilResponse.getEnquirySegment().getEnquiryBeanList(), mobRefNo, leadID,srNo);
			}
			
			if(cibilResponse.getConsumerDisputeRemarksSegment()!=null)
			{
				this.storeConsumerDisputeRemarksSegment(cibilResponse.getConsumerDisputeRemarksSegment(), mobRefNo, leadID,srNo);
			}
	...
	public void storeConsumerDisputeRemarksSegment(ConsumerDisputeRemarksSegment consumerDisputeRemarksSegment,String mobRefNo,String leadID,long srNo)
	{
	...
	public void storeAccountSegmentOthersAccounts(List<OthersAccountBean> othersAccountBeanList,String mobRefNo,String leadID,long srNo)
	{
	...
	public void storeAccountSegmentOwnAccounts(List<OwnAccountBean> ownAccountBeanList,String mobRefNo,String leadID,long srNo)
	{
	...
	public void storeEnquiryAccountNumberSegment(List<AccountNumberBean> accNumBeanList,String mobRefNo,String leadID,long srNo)
	{
	...
	{
		String query = "insert into ILT_IQ_CIBIL_IDENTIFI_SGMT(APP_PERS_SR_NO,CIBIL_MOB_REF_NO,CIBIL_LEAD_ID,CIBIL_ID_TYPE,CIBIL_ID_NUMBER,CIBIL_ISSUE_DATE,CIBIL_EXPIRATION_DATE,CIBIL_ENRICHED_THRO_ENQY) values(?,?,?,?,?,?,?,?)";
	...
	/*public int insertPennantCibilResponse(PennantRequest req) {
		int status = 0;
		//int id=0;

		String query = "insert into cust_api_cibil_data(CIBILSRNO,CUSTOMERNAME,PANNUMBER,DATEOFBIRTH,DATEPROCESSED,MEMBER_REF_NUM,"
				+ "TIMEPROCESSED,SCORENAME,SCORECARDNAME,SCORECARDVERSION,SCORE,CIBILRESPONSE) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		
		try {
			Connection con = JdbcSQLServerConnection.getConnection();
			//PreparedStatement st = con.prepareStatement(query);
			//PreparedStatement st=con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
			CallableStatement st=con.prepareCall(query);
			st.setString(1, req.getCibilSrNo());
			st.setString(2, req.getCustomerName());
			st.setString(3, req.getPan());
			st.setDate(4, new java.sql.Date(req.getDob().getTime()));
			st.setString(5, req.getDateProcessed());
			st.setString(6, req.getMemRefNo());
			st.setString(7, req.getTimeProcessed());
			st.setString(8, req.getScoreName());
			st.setString(9, req.getScoreCardName());
			st.setString(10, req.getScoreCardVersion());
			st.setString(11, req.getScore());
	...
	public int insertCibilResponse(StringBuilder inputCSV,NewCibilRequest ncr,String output,String inputRequest,String headerInfo, String ip, Map<String, Object> existMapData,String user_id) {
		int status = 0;
		int id=0;

		String query = "begin insert into ilt_iq_app_pers_cibil(APP_PERS_MOB_REF_NO,APP_PERS_LEAD_ID,APP_PERS_INPUT_CSV,APP_PERS_CIBIL_RESP,APP_PERS_CIBIL_REMARKS,APP_PERS_CIBIL_FLAG,APP_PERS_CIBIL_DATE,APP_PERS_CLIENT_REQ,APP_PERS_REQ_HEADER,APP_PERS_INITIATED_DATE,APP_PERS_IP_ADDRESS,APP_PERS_INITIATED_BY) values(?,?,?,?,?,?,?,?,?,SYSDATE,?,?) RETURNING APP_PERS_SR_NO INTO ?;end;";
	 ...
	 * 
	 * These lines of code store inputs and error output in db for further reference.
	 * 
	 ...
	 * 
	 * This method stores cibil report in Base64 String in db clob column against given sr no id.
	 */
	 ...
	 * @param score
	 * @param userId
	 * @return This method return cibil remarks and band values against score and userId.
	 */
	public Map<String,String> getCibilRemarksForScore(int score,String userId) {
		...
		String query = "select remarks,band_value from ilt_ap_cibil_remark where ? between from_score and to_score and trunc(sysdate) between effective_date and to_date and API_USER_ID=?";
		try {
			Connection con = ConnectionFactory.getConnection();
			PreparedStatement st = con.prepareStatement(query);
			st.setInt(1, score);
	 ...
	 * 
	 * <p>This method validates users auth key, userId and resp_type against db 
	 * and returns restriction message,DBException message or api user status such as 'active' or 'deactive'.</p>
	 * 
	 ...
	 * @return This method checks compare given input csv with db table.So that there will not multiple hits to cibil server for same request in day.
	 */
	public Map<String,Object> findCibilRespByInputCSVForCurrentDay(StringBuilder inputCSV) {
		...
		String query = "select app_pers_cibil_resp,app_pers_cibil_date from ilt_iq_app_pers_cibil where app_pers_input_csv = ? and trunc(app_pers_initiated_date)= trunc(sysdate) and app_pers_cibil_remarks='SUCCESS' and rownum  = 1 order by app_pers_sr_no desc";
		try {
			Connection con = ConnectionFactory.getConnection();
			PreparedStatement st = con.prepareStatement(query);
			...
			while(rs.next())
			{
				cibil_resp=rs.getString("app_pers_cibil_resp")!=null?rs.getString("app_pers_cibil_resp").trim():null;
	...
	public Map<String,Object> getExistingCibilResponse(String srNo) {
		...
		try {
			...
			while(rs.next())
			{	
				mapData.put("output", rs.getString("app_pers_cibil_resp")!=null?rs.getString("app_pers_cibil_resp").trim():null);
